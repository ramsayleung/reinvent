...
describe('pre-existing hashes and actual filesystem', () => {
  it('finds no pre-existing files when none given or eixst', async () => {
    const expected = {}
    const actual = await findNew('file_backup/test/bck-0-csv-0', []);
    assert.deepStrictEqual(actual, expected, 'Expected no files');
  })

  it('find some files when one is given and none exist', async () => {
    const check = [['somefile.txt', '9876fedc']];
    const expected = { '9876fedc': 'somefile.txt' }
    const actual = await findNew('file_backup/test/bck-0-csv-0', check);
    assert.deepStrictEqual(actual, expected, 'Expected one file');
  })

  it('finds nothing needs backup when there is a match', async () => {
    const check = [['alpha.js', 'abcd1234']];
    const expected = {};
    const actual = await findNew('file_backup/test/bck-1-csv-1', check);
    assert.deepStrictEqual(actual, expected, 'Expected no files')
  })
  ...
  it('finds mixed matches', async () => {
    ...
    const actual = await findNew('file_backup/test/bck-4-csv-2', check);
    assert.deepStrictEqual(actual, expected, 'Expected one file');
  })
})
