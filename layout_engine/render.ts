...
export interface Node extends Renderable, Placeable {
  x0: number;
  y0: number;
  children?: Node[];
}
...
const draw = (screen: string[][], node: Node, fill: string = null): string => {
  fill = nextFill(fill);
...
const drawBlock = (screen: string[][], node: Node, fill: string) => {
...
export class RenderedBlock extends PlacedBlock implements Node {
  render(screen: string[][], fill: string) {
    drawBlock(screen, this, fill);
  }
}

export class RenderedCol extends PlacedCol implements Node {
...
export class RenderedRow extends PlacedRow implements Node {
  // We need to override the children property to make it compatible with Node
  declare children: Node[];
